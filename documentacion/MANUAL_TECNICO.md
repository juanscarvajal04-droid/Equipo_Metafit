...
- **Diseño del correo**: plantilla HTML profesional en `backend/templates/recuperar-password.html` (estilos 100 % inline — obligatorio en clientes de correo —, fondo oscuro `#0a0a0f`, texto `#ffffff`, botón `#7c3aed` redondeado, sans-serif, responsive hasta 600 px). Logo de MetaFit (mancuerna roja `#e31c25`) como **PNG público de 512 px**: `frontend_web/public/app/logo-metafit.png` → `…/app/logo-metafit.png` (mejor compatibilidad que SVG en Outlook/Gmail). `{{NOMBRE}}`, `{{ENLACE}}` y `{{ANIO}}` se reemplazan en `authController.js`; si la plantilla no se puede leer, el correo cae a texto plano. El remitente es **"MetaFit" &lt;metafit.sistema@gmail.com&gt;**.

**✅ Prueba en producción (ago 2026):** con `BREVO_API_KEY` validada contra `api.brevo.com/v3/account` se creó el usuario temporal `Test` (`metafit.sistema@gmail.com`), se solicitó la recuperación y el endpoint respondió **200 sin `modoPrueba`** — el correo se despachó por la **API REST Brevo (HTTPS)** en ~1.3 s con el diseño HTML (logo, botón, colores) y Brevo registró el evento **delivered**. El usuario temporal se eliminó y la BD quedó limpia. El enlace usa `FRONTEND_URL` + `/#/reset-password/{token}` (HashRouter: el sitio estático no hace fallback SPA).
