...
// FASE A.2: además calcula macronutrientes con la misma proporción:
//   macro_consumido = macro_por_100g x cantidad_g_consumida / 100.
'use strict';
...
const ConsumoAlimentoRealModel = {
  ...
  // Calorías y macros se derivan en la MISMA query desde ALIMENTO (evita doble roundtrip).
  insertar: async (conn, datos) => {
    ...
    const [r] = await conn.query(
      `INSERT INTO CONSUMO_ALIMENTO_REAL
       ...
       SELECT ?, ?, ?, ?, ?, ?,
              ...
              ROUND(a.proteinas     * ? / 100, 2),
              ROUND(a.carbohidratos * ? / 100, 2),
              ROUND(a.grasas        * ? / 100, 2)
       FROM ALIMENTO a
       WHERE a.id_alimento = ?
       LIMIT 1`,
      [id_usuario, id_ciclo, num_comida, id_alimento, fecha,
       cantidad_g_consumida, cantidad_g_consumida,
       cantidad_g_consumida, cantidad_g_consumida, cantidad_g_consumida,
       id_alimento]
    );
    if (r.affectedRows === 0) {
    ...
    // Recuperar los valores calculados para devolverlos al cliente
    const [cal] = await conn.query(
      `SELECT calorias_consumidas,
              proteinas_consumidas, carbohidratos_consumidos, grasas_consumidas
       FROM CONSUMO_ALIMENTO_REAL WHERE id_consumo = ?`,
      [r.insertId]
    );
    return {
      id_consumo:                 r.insertId,
      calorias_consumidas:        cal.length ? cal[0].calorias_consumidas        : null,
      proteinas_consumidas:       cal.length ? cal[0].proteinas_consumidas       : null,
      carbohidratos_consumidos:   cal.length ? cal[0].carbohidratos_consumidos   : null,
      grasas_consumidas:          cal.length ? cal[0].grasas_consumidas          : null,
    };
  },
  ...
  getHistorial: async (id_usuario, { id_ciclo, fechaInicio, fechaFin }) => {
    let sql = `
      SELECT c.id_consumo, c.id_ciclo, c.num_comida, c.fecha,
             ...
             c.proteinas_consumidas, c.carbohidratos_consumidos, c.grasas_consumidas,
